"""VPE-Syntax installation script for Linux and Windows"""
...
def main(args: argparse.Namespace) -> None:
    ...
    # Install VPE-Syntax, which will pull in the dependencies.
    print('Installing VPE-Sitter and dependencies...')
    if not run_command(pip_base + ['--upgrade', 'vpe-syntax']):
        sys.exit(1)

    # If requested, install language parsers.
    if args.add_languages:
        print('Installing parsers for Python and C...')
        if not run_command(pip_base + [
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Install VPE-Syntax and dependencies.')
    parser.add_argument(
        '--make-vim-dir', action='store_true',
        help='Create Vim config directory if required.')
    parser.add_argument(
        '--add-languages', action='store_true',
        help='Install C and Python language parsers.')
    parser.add_argument(
        '--vim-path',
